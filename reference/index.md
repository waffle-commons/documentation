...
| **Data** | `waffle-commons/data` | Worker-safe persistence (RFC-022): `PDOConnectionPool`, backend-agnostic query AST (SQR), compilers for all six SQL dialects + Firestore/MongoDB/key-value/Cassandra/GraphQL, typed stateless repositories, live Redis/Mongo/PSR-18 drivers, atomic flat-file JSON store, property-hook hydrator, `db:migrate` migration runner. | [data.md](data.md) |
