    ...
    public string $email {
        set(string $value) {
            if (!filter_var($value, FILTER_VALIDATE_EMAIL)) {
                // Throwing a typed ValidationException preserves the `field`
                // slot in the rendered 422 payload.
                throw new ValidationException(message: 'Invalid email.', field: 'email');
            }
